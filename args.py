...
class ConfigForm:
    def __init__(self, callback):
        ...
        self.session_label = Label(master, text="Saved Profile Sessions (space-separated):")
        ...
        if latest_config:
            if latest_config.browsers:
                self.browser_entry.insert(0, args.b or ' '.join(map(str, latest_config.browsers.split())))
            if latest_config.profiles:
                self.profile_entry.insert(0, args.p or ' '.join(map(str, latest_config.profiles.split())))
            if latest_config.sessions:
                self.session_entry.insert(0, args.s or ' '.join(map(str, latest_config.sessions.split())))
    ...
    def submit(self):
        ...
        browsers = list(map(int, self.browser_entry.get().split())) if self.browser_entry.get() else None
        profiles = list(map(int, self.profile_entry.get().split())) if self.profile_entry.get() else None
        sessions = list(map(int, self.session_entry.get().split())) if self.session_entry.get() else None
        working_dir = self.working_dir_entry.get() if self.working_dir_entry.get() else None
        ...
        print(config)
        new_config = Config()
        new_config.browsers = ' '.join(map(str, config['browsers'])) if config['browsers'] else None
        new_config.profiles = ' '.join(map(str, config['profiles'])) if config['profiles'] else None
        new_config.sessions = ' '.join(map(str, config['sessions'])) if config['sessions'] else None
        new_config.working_dir = config['working_dir'] if config['working_dir'] else None
        new_config.save()
